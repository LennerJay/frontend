  ...
  >
    ...
    >
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center gap-5">
            ...
            <transition name="fade">
              <span class="w-full pl-3 text-rose-600" v-if="errors.description">{{
                errors.description
              }}</span>
            </transition>
          </div>
      ...
      <div class="flex justify-end mt-5 mb-5 gap-6">
        ...
        <button class="border border-black p-1" id="back-button" @click="saveCriteria">
          Save
        </button>
      </div>
...
const description = props.criteria ? ref(props.criteria.description) : ref("");

const errors = ref({});
const emits = defineEmits(["close"]);
const props = defineProps(["criteria", "option"]);
...
const saveCriteria = async () => {
  ...
  if (props.option == "Add") {
    await criteriaStore.saveCriteria(description.value);
  } else {
    await criteriaStore.saveUpdateCriteria(props.criteria.id, description.value);
  }
...
const closeModal = () => {
  if(description.value == "" || description.value == undefined && props.option == "Add"){
    emits("close");
